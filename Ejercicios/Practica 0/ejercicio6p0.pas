{
Realice un programa que informe el valor total en pesos de una transacción en dólares. Para
ello, el programa debe leer el monto total en dólares de la transacción, el valor del dólar al día
de la fecha y el porcentaje (en pesos) de la comisión que cobra el banco por la transacción. Por
ejemplo, si la transacción se realiza por 10 dólares, el dólar tiene un valor 189,32 pesos (hace 3000 años)
y el banco cobra un 4% de comisión, entonces el programa deberá informar:

La transacción será de 1968,93 pesos argentinos

(Resultado de multiplicar 10*189,32 y adicionarle el 4%)
}

program ej06p0;
const
	comision = 0.04;
	valor = 189.32;
var
	dolar,transaccion:real;
begin
	writeln('Ingresar total de dolares');
	readln(dolar);
	
	transaccion := dolar * valor;
	transaccion := (transaccion  + (transaccion * comision));
	writeln('El total de la transaccion es de:', transaccion);
end.
